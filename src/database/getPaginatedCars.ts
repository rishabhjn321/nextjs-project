...
export async function getPaginatedCars(query: ParsedUrlQuery) {
  ...
  const page = getValueNumber(query.page) || 1; // 1 = rows
  ...
  const carsPromise = db.all<CarModel[]>(
    `SELECT * ${mainQuery} LIMIT @rowsPerPage OFFSET @offset`, // limit = get me the next offset = jump this many row
    {
      ...dbParams,
  ...
  return { cars, totalPages: Math.ceil(totalRows.count / rowsPerPage) }; // math.ceil if its showing 10.3 it will show 11
}
